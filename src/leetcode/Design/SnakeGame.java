...
public class SnakeGame {
	...
	public int move(String direction) {
		...
		for(int i = 1; i < snake.size() - 1; i++){
			...
			if(next.isEqual(cur)) return -1;	    //snake eats body    
		}
		snake.addFirst(cur);         //add new Head to snake
		if(len < food.length) {
		...
		while(snake.size() > len+1) snake.removeLast(); //if snake did not have food, remove tail
		return len;
	}
}
